#ActionableItems, decision, question

from langchain_mistralai import ChatMistralAI
from langchain_groq import ChatGroq
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.output_parsers import StrOutputParser
from langchain_core.runnables import RunnablePassthrough, RunnableLambda

import os

# def get_llm():
#   return ChatMistralAI(model="mistral-small-latest",mistral_api_key= os.getenv("MISTRAL_API_KEY"),temperature=0.2)

def get_llm():
  return ChatGroq(model="openai/gpt-oss-120b",groq_api_key= os.getenv("GROQ_API_KEY"),temperature=0.2)

def build_chain(system_prompt : str):
    llm = get_llm()
    return (
        RunnablePassthrough() | RunnableLambda(lambda x : {"text" : x}) |ChatPromptTemplate.from_messages([
        ("system", system_prompt),
        ("human","{text}"),
    ]) | llm |StrOutputParser()
    )


def extract_action_item(transcript:str)->str:
   chain = build_chain(
         "You are an expert meeting analyst. From the meeting transcript, "
        "extract all action items. For each provide:\n"
        "- Task description\n"
        "- Owner (who is responsible)\n"
        "- Deadline (if mentioned, else write 'Not specified')\n\n"
        "Format as a numbered list. If none found say 'No action items found.'"
    )
   return chain.invoke(transcript)


# def extract_key_decisions(transcript: str) -> str:
#     chain = build_chain(
#         "You are an expert meeting analyst. From the meeting transcript, "
#         "extract all key decisions made. Format as a numbered list. "
#         "If none found say 'No key decisions found.'"
#     )
#     return chain.invoke(transcript)

import time


def extract_key_decisions(transcript: str) -> str:

    chain = build_chain(
        "You are an expert meeting analyst. "
        "Extract all key decisions from the transcript. "
        "Return them as a numbered list. "
        "If there are no decisions, return 'No key decisions found.'"
    )

    # Process transcript in smaller chunks
    chunk_size = 2500

    chunks = [
        transcript[i:i + chunk_size]
        for i in range(0, len(transcript), chunk_size)
    ]

    all_decisions = []

    for i, chunk in enumerate(chunks):

        print(f"Processing decision chunk {i + 1}/{len(chunks)}...")

        try:
            result = chain.invoke(chunk)

            if result and "No key decisions found." not in result:
                all_decisions.append(result)

            # Wait to avoid Groq TPM rate limit
            if i < len(chunks) - 1:
                time.sleep(3)

        except Exception as e:
            print(f"Error processing chunk {i + 1}: {e}")

    if not all_decisions:
        return "No key decisions found."

    return "\n".join(all_decisions)

def extract_questions(transcript: str) -> str:
    chain = build_chain(
        "From the meeting transcript, extract all unresolved questions "
        "or topics needing follow-up. Format as a numbered list. "
        "If none found say 'No open questions found.'"
    )
    return chain.invoke(transcript)